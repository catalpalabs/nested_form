...
I need a simple way to support doubly nested forms containing associations with the same name (related to issue #45, I believe). In the case where:
...
A nested contact form (within the user form) can set the :association_name explicitly to avoid duplication of the address association mechanism:

  <%= f.fields_for :addresses do |builder| %>
    <%= render 'shared/address_fields', :f => builder %>
  <% end %>
  <%= f.link_to_add 'add another', :addresses, :association_name => :contact_addresses %>
